...
export default function NalogPregled() {
    ...
    // --- LOGIKA: FILTRIRANJE I SORTIRANJE (koristimo useMemo za performanse) ---
    const obradjeniNalozi = useMemo(() => {
        // 1. Filtriranje
        const filtrirani = nalozi.filter(n => {
            ...
            return nazivP.includes(search) || nazivG.includes(search) || n.sifra.toString().includes(search);
        });
    ...
    }, [nalozi, filterTekst, sortKey, sortOrder, poduzeca, gradilista]);

    // --- IZRAČUN UKUPNOG ZBROJA (uvijek nad svim filtriranim rezultatima) ---
    const ukupnaSuma = useMemo(() => {
        return obradjeniNalozi.reduce((acc, n) => {
            const iznos = parseFloat(n.ukupniIznos) || 0;
            return acc + iznos;
        }, 0);
    }, [obradjeniNalozi]);
    ...
    const currentItems = obradjeniNalozi.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(obradjeniNalozi.length / itemsPerPage);
    ...
    const ispisPDF = () => {
        ...
        const tableRows = obradjeniNalozi.map(n => [
            n.sifra,
            dohvatiNazivPoduzeca(n.sifraPoduzeca),
            dohvatiNazivGradilista(n.sifraGradilista),
            new Intl.NumberFormat('hr-HR', { style: 'currency', currency: 'EUR' }).format(n.ukupniIznos || 0)
        ]);
            ...
            <Row className="mb-3 align-items-end">
                ...
                <Col md={2}>
                    <Button variant="info" className="w-100" onClick={ispisPDF}>
                        <FaPrint className="me-2" /> Ispiši PDF
                    </Button>
                </Col>
                    ...
                    {currentItems.length > 0 ? (
                        <>
                            ...
                            <tr className="table-secondary">
                                <td colSpan="3" className="text-end fw-bold">UKUPNO (na svim stranicama):</td>
